...
class Schedule {
	...
	function getWhereSql($p) {
		$whereSql = " WHERE 1=1 AND sche_delyn = 'N'";
...
// 		}
	...
	function getList($param='') {
		...
		foreach($list as $key => $val){
			$member_name = getNmemberToText($list[$key]['sche_nmem']);
			$sdata[$key]['title'] = $list[$key]['sche_contents'].' ('.$member_name.')';
			$sdata[$key]['start'] = $list[$key]['sche_date'];
			$sdata[$key]['sinfo'] = $list[$key];
		}
	...
	function delete($no=0) {
		...
		$sql = "UPDATE ".$this->tableName." SET sche_delyn = 'Y' WHERE sche_id = ".$no;
